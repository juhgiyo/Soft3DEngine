///////////////////////////////////////////////////////////
//                                                       //
//                    WgQuaternion.h                     //
//                                                       //
//  - Interface for Plane 3 class                        //
//                                                       //
//  - Written By Woong Gyu La a.k.a. Chris               //
//       on 2009.10.07                                   //
//                                                       //
///////////////////////////////////////////////////////////

#ifndef __WG_QUATERNION_H__
#define __WG_QUATERNION_H__

#include "WgFoundationLIB.h"
#include "WgMatrix3.h"

namespace WGSoft3D
{

template <class Real>
class Quaternion
{
public:
    // A quaternion is q = w + x*i + y*j + z*k where (w,x,y,z) is not
    // necessarily a unit length vector in 4D.

    // construction
    Quaternion ();  // uninitialized
    Quaternion (Real fW, Real fX, Real fY, Real fZ);
    Quaternion (const Quaternion& rkQ);

    // quaternion for the input rotation matrix
    Quaternion (const Matrix3<Real>& rkRot);

    // quaternion for the rotation of the axis-angle pair
    Quaternion (const Vector3<Real>& rkAxis, Real fAngle);

    // quaternion for the rotation matrix with specified columns
    Quaternion (const Vector3<Real> akRotColumn[3]);

    // member access:  0 = w, 1 = x, 2 = y, 3 = z
    operator const Real* () const;
    operator Real* ();
    Real operator[] (int i) const;
    Real& operator[] (int i);
    Real W () const;
    Real& W ();
    Real X () const;
    Real& X ();
    Real Y () const;
    Real& Y ();
    Real Z () const;
    Real& Z ();

    // assignment
    Quaternion& operator= (const Quaternion& rkQ);

    // comparison
    bool operator== (const Quaternion& rkQ) const;
    bool operator!= (const Quaternion& rkQ) const;
    bool operator<  (const Quaternion& rkQ) const;
    bool operator<= (const Quaternion& rkQ) const;
    bool operator>  (const Quaternion& rkQ) const;
    bool operator>= (const Quaternion& rkQ) const;

    // arithmetic operations
    Quaternion operator+ (const Quaternion& rkQ) const;
    Quaternion operator- (const Quaternion& rkQ) const;
    Quaternion operator* (const Quaternion& rkQ) const;
    Quaternion operator* (Real fScalar) const;
    Quaternion operator/ (Real fScalar) const;
    Quaternion operator- () const;

    // arithmetic updates
    Quaternion& operator+= (const Quaternion& rkQ);
    Quaternion& operator-= (const Quaternion& rkQ);
    Quaternion& operator*= (Real fScalar);
    Quaternion& operator/= (Real fScalar);

    // conversion between quaternions, matrices, and axis-angle
    Quaternion& FromRotationMatrix (const Matrix3<Real>& rkRot);
    void ToRotationMatrix (Matrix3<Real>& rkRot) const;
    Quaternion& FromRotationMatrix (const Vector3<Real> akRotColumn[3]);
    void ToRotationMatrix (Vector3<Real> akRotColumn[3]) const;
    Quaternion& FromAxisAngle (const Vector3<Real>& rkAxis, Real fAngle);
    void ToAxisAngle (Vector3<Real>& rkAxis, Real& rfAngle) const;

    // functions of a quaternion
    Real Length () const;  // length of 4-tuple
    Real SquaredLength () const;  // squared length of 4-tuple
    Real Dot (const Quaternion& rkQ) const;  // dot product of 4-tuples
    Real Normalize ();  // make the 4-tuple unit length
    Quaternion Inverse () const;  // apply to non-zero quaternion
    Quaternion Conjugate () const;
    Quaternion Exp () const;  // apply to quaternion with w = 0
    Quaternion Log () const;  // apply to unit-length quaternion

    // rotation of a vector by a quaternion
    Vector3<Real> Rotate (const Vector3<Real>& rkVector) const;

    // spherical linear interpolation
    Quaternion& Slerp (Real fT, const Quaternion& rkP, const Quaternion& rkQ);

    Quaternion& SlerpExtraSpins (Real fT, const Quaternion& rkP,
        const Quaternion& rkQ, int iExtraSpins);

    // intermediate terms for spherical quadratic interpolation
    Quaternion& Intermediate (const Quaternion& rkQ0,
        const Quaternion& rkQ1, const Quaternion& rkQ2);

    // spherical quadratic interpolation
    Quaternion& Squad (Real fT, const Quaternion& rkQ0,
        const Quaternion& rkA0, const Quaternion& rkA1,
        const Quaternion& rkQ1);

    // Compute a quaternion that rotates unit-length vector V1 to unit-length
    // vector V2.  The rotation is about the axis perpendicular to both V1 and
    // V2, with angle of that between V1 and V2.  If V1 and V2 are parallel,
    // any axis of rotation will do, such as the permutation (z2,x2,y2), where
    // V2 = (x2,y2,z2).
    Quaternion& Align (const Vector3<Real>& rkV1, const Vector3<Real>& rkV2);

    // Decompose a quaternion into q = q_twist * q_swing, where q is 'this'
    // quaternion.  If V1 is the input axis and V2 is the rotation of V1 by
    // q, q_swing represents the rotation about the axis perpendicular to
    // V1 and V2 (see Quaternion::Align), and q_twist is a rotation about V1.
    void DecomposeTwistTimesSwing (const Vector3<Real>& rkV1,
        Quaternion& rkTwist, Quaternion& rkSwing);

    // Decompose a quaternion into q = q_swing * q_twist, where q is 'this'
    // quaternion.  If V1 is the input axis and V2 is the rotation of V1 by
    // q, q_swing represents the rotation about the axis perpendicular to
    // V1 and V2 (see Quaternion::Align), and q_twist is a rotation about V1.
    void DecomposeSwingTimesTwist (const Vector3<Real>& rkV1,
        Quaternion& rkSwing, Quaternion& rkTwist);

    // special values
    WG3D_FOUNDATION_ITEM static const Quaternion IDENTITY;  // the identity rotation
    WG3D_FOUNDATION_ITEM static const Quaternion ZERO;

private:
    // support for comparisons
    int CompareArrays (const Quaternion& rkQ) const;

    // support for FromRotationMatrix
    static int ms_iNext[3];

    Real m_afTuple[4];
};


class Quaternionx
{
public:
    // A quaternion is q = w + x*i + y*j + z*k where (w,x,y,z) is not
    // necessarily a unit length vector in 4D.

    // construction
    Quaternionx ();  // uninitialized
    Quaternionx (fixed fW, fixed fX, fixed fY, fixed fZ);
    Quaternionx (const Quaternionx& rkQ);

    // quaternion for the input rotation matrix
    Quaternionx (const Matrix3x& rkRot);

    // quaternion for the rotation of the axis-angle pair
    Quaternionx (const Vector3x& rkAxis, fixed fAngle);

    // quaternion for the rotation matrix with specified columns
    Quaternionx (const Vector3x akRotColumn[3]);

    // member access:  0 = w, 1 = x, 2 = y, 3 = z
    operator const fixed* () const;
    operator fixed* ();
    fixed operator[] (int i) const;
    fixed& operator[] (int i);
    fixed W () const;
    fixed& W ();
    fixed X () const;
    fixed& X ();
    fixed Y () const;
    fixed& Y ();
    fixed Z () const;
    fixed& Z ();

    // assignment
    Quaternionx& operator= (const Quaternionx& rkQ);

    // comparison
    bool operator== (const Quaternionx& rkQ) const;
    bool operator!= (const Quaternionx& rkQ) const;
    bool operator<  (const Quaternionx& rkQ) const;
    bool operator<= (const Quaternionx& rkQ) const;
    bool operator>  (const Quaternionx& rkQ) const;
    bool operator>= (const Quaternionx& rkQ) const;

    // arithmetic operations
    Quaternionx operator+ (const Quaternionx& rkQ) const;
    Quaternionx operator- (const Quaternionx& rkQ) const;
    Quaternionx operator* (const Quaternionx& rkQ) const;
    Quaternionx operator* (fixed fScalar) const;
    Quaternionx operator/ (fixed fScalar) const;
    Quaternionx operator- () const;

    // arithmetic updates
    Quaternionx& operator+= (const Quaternionx& rkQ);
    Quaternionx& operator-= (const Quaternionx& rkQ);
    Quaternionx& operator*= (fixed fScalar);
    Quaternionx& operator/= (fixed fScalar);

    // conversion between quaternions, matrices, and axis-angle
    Quaternionx& FromRotationMatrix (const Matrix3x& rkRot);
    void ToRotationMatrix (Matrix3x& rkRot) const;
    Quaternionx& FromRotationMatrix (const Vector3x akRotColumn[3]);
    void ToRotationMatrix (Vector3x akRotColumn[3]) const;
    Quaternionx& FromAxisAngle (const Vector3x& rkAxis, fixed fAngle);
    void ToAxisAngle (Vector3x& rkAxis, fixed& rfAngle) const;

    // functions of a quaternion
    fixed Length () const;  // length of 4-tuple
    fixed SquaredLength () const;  // squared length of 4-tuple
    fixed Dot (const Quaternionx& rkQ) const;  // dot product of 4-tuples
    fixed Normalize ();  // make the 4-tuple unit length
    Quaternionx Inverse () const;  // apply to non-zero quaternion
    Quaternionx Conjugate () const;
    Quaternionx Exp () const;  // apply to quaternion with w = 0
    Quaternionx Log () const;  // apply to unit-length quaternion

    // rotation of a vector by a quaternion
    Vector3x Rotate (const Vector3x& rkVector) const;

    // spherical linear interpolation
    Quaternionx& Slerp (fixed fT, const Quaternionx& rkP, const Quaternionx& rkQ);

    Quaternionx& SlerpExtraSpins (fixed fT, const Quaternionx& rkP,
        const Quaternionx& rkQ, int iExtraSpins);

    // intermediate terms for spherical quadratic interpolation
    Quaternionx& Intermediate (const Quaternionx& rkQ0,
        const Quaternionx& rkQ1, const Quaternionx& rkQ2);

    // spherical quadratic interpolation
    Quaternionx& Squad (fixed fT, const Quaternionx& rkQ0,
        const Quaternionx& rkA0, const Quaternionx& rkA1,
        const Quaternionx& rkQ1);

    // Compute a quaternion that rotates unit-length vector V1 to unit-length
    // vector V2.  The rotation is about the axis perpendicular to both V1 and
    // V2, with angle of that between V1 and V2.  If V1 and V2 are parallel,
    // any axis of rotation will do, such as the permutation (z2,x2,y2), where
    // V2 = (x2,y2,z2).
    Quaternionx& Align (const Vector3x& rkV1, const Vector3x& rkV2);

    // Decompose a quaternion into q = q_twist * q_swing, where q is 'this'
    // quaternion.  If V1 is the input axis and V2 is the rotation of V1 by
    // q, q_swing represents the rotation about the axis perpendicular to
    // V1 and V2 (see Quaternionx::Align), and q_twist is a rotation about V1.
    void DecomposeTwistTimesSwing (const Vector3x& rkV1,
        Quaternionx& rkTwist, Quaternionx& rkSwing);

    // Decompose a quaternion into q = q_swing * q_twist, where q is 'this'
    // quaternion.  If V1 is the input axis and V2 is the rotation of V1 by
    // q, q_swing represents the rotation about the axis perpendicular to
    // V1 and V2 (see Quaternionx::Align), and q_twist is a rotation about V1.
    void DecomposeSwingTimesTwist (const Vector3x& rkV1,
        Quaternionx& rkSwing, Quaternionx& rkTwist);

    // special values
    WG3D_FOUNDATION_ITEM static const Quaternionx IDENTITY;  // the identity rotation
    WG3D_FOUNDATION_ITEM static const Quaternionx ZERO;

private:
    // support for comparisons
    int CompareArrays (const Quaternionx& rkQ) const;

    // support for FromRotationMatrix
    static int ms_iNext[3];

    fixed m_afTuple[4];
};

template <class Real>
Quaternion<Real> operator* (Real fScalar, const Quaternion<Real>& rkQ);

inline Quaternionx operator* (fixed fScalar, const Quaternionx& rkQ);
#include "WgQuaternion.inl"

typedef Quaternion<float> Quaternionf;
typedef Quaternion<double> Quaterniond;

}

#endif

