...
#ifndef __WG_FRUSTUM3_H__
#define __WG_FRUSTUM3_H__
...
class WG3D_FOUNDATION_ITEM Frustum3x
{
public:
    // Orthogonal frustum.  Let E be the origin, D be the direction vector,
    // U be the up vector, and R be the right vector.  Let u > 0 and r > 0 be
    ...
    // R = (1,0,0), DMin = 1, DMax = 2, UBound = 1, RBound = 1.
    Frustum3x ();
    Frustum3x (const Vector3x& rkOrigin, const Vector3x& rkDVector,
        const Vector3x& rkUVector, const Vector3x& rkRVector,
        fixed fDMin, fixed fDMax, fixed fUBound, fixed fRBound);
    ...
    // Update() call.
    void Update ();
    fixed GetDRatio () const;
    fixed GetMTwoUF () const;
    fixed GetMTwoRF () const;
	
    void ComputeVertices (Vector3x akVertex[8]) const;
	
    Vector3x Origin, DVector, UVector, RVector;
    fixed DMin, DMax, UBound, RBound;
	
protected:
    // derived quantities
    fixed m_fDRatio, m_fMTwoUF, m_fMTwoRF;
};
#include "WgFrustum3.inl"
