			   ...
			   fixed upx, fixed upy, fixed upz)
{
	...
	/* Z vector */
	z[0] = FloatFromFixed(eyex) - FloatFromFixed(centerx);
	z[1] = FloatFromFixed(eyey) - FloatFromFixed(centery);
	z[2] = FloatFromFixed(eyez) - FloatFromFixed(centerz);
	...
	/* Y vector */
	y[0] = FloatFromFixed(upx);
	y[1] = FloatFromFixed(upy);
	y[2] = FloatFromFixed(upz);
...
#define M(row,col)  m[col*4+row]
	M(0, 0) = FixedFromFloat(x[0]);
	M(0, 1) = FixedFromFloat(x[1]);
	M(0, 2) = FixedFromFloat(x[2]);
	M(0, 3) = FIXED_ZERO;
	M(1, 0) = FixedFromFloat(y[0]);
	M(1, 1) = FixedFromFloat(y[1]);
	M(1, 2) = FixedFromFloat(y[2]);
	M(1, 3) = FIXED_ZERO;
	M(2, 0) = FixedFromFloat(z[0]);
	M(2, 1) = FixedFromFloat(z[1]);
	M(2, 2) = FixedFromFloat(z[2]);
	M(2, 3) = FIXED_ZERO;
	M(3, 0) = FIXED_ZERO;
	M(3, 1) = FIXED_ZERO;
	M(3, 2) = FIXED_ZERO;
